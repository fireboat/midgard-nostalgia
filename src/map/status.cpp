...
void status_calc_misc(struct block_list *bl, struct status_data *status, int32 level)
{
	...
	if (bl->type == BL_HOM) {
		...
		// Hit
		stat = status->hit;
		stat += (bl->type == BL_MOB ? level/2 : level) + status->dex + (bl->type == BL_PC ? status->luk / 3 + 175 : 150); //base level + ( every 1 dex = +1 hit ) + (every 3 luk = +1 hit) + 175
		stat += 2 * status->con;
		status->hit = cap_value(stat, 1, SHRT_MAX);
		// Flee
		stat = status->flee;
		stat += (bl->type == BL_MOB ? level / 2 : level) + status->agi + (bl->type == BL_MER ? 0 : bl->type == BL_PC ? status->luk / 5 : 0) + 100; //base level + ( every 1 agi = +1 flee ) + (every 5 luk = +1 flee) + 100
		stat += 2 * status->con;
		status->flee = cap_value(stat, 1, SHRT_MAX);
		// Def2
		if (bl->type == BL_MER)
	...
	if( sd && sd->ud.skilltimer != INVALID_TIMER && (pc_checkskill(sd,SA_FREECAST) > 0 || sd->ud.skill_id == LG_EXEEDBREAK) ) {
		...
		if( sd && sc->getSCE(SC_HIDING) && pc_checkskill(sd,RG_TUNNELDRIVE) > 0 )
			...
			if( sd && sc->getSCE(SC_DANCING) )
				val = max( val, 400 - (35 + 5 * (sc->getSCE(SC_SPIRIT) && sc->getSCE(SC_SPIRIT)->val2 == SL_BARDDANCER)) * pc_checkskill(sd,(sd->status.sex?BA_MUSICALLESSON:DC_DANCINGLESSON)) );
			if( sc->getSCE(SC_DECREASEAGI) )
				val = max( val, 25 );
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_SACRIFICE:
			...
			break;
		case SC_ENCPOISON:
			val2= 200*val1; // Poisoning Chance (2.5+0.5%) in 1/10000 rate
			break;
